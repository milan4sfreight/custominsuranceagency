...
export default function FloodInsuranceQuote() {
  ...
  const [hasFlood, setHasFlood] = useState(""); const [expiration, setExpiration] = useState("");

  const [coverageFor, setCoverageFor] = useState(""); const [yearsLived, setYearsLived] = useState("");
  const [dwellingValue, setDwellingValue] = useState(""); const [occupiedBy, setOccupiedBy] = useState("");

  const [structure, setStructure] = useState(""); const [yearBuilt, setYearBuilt] = useState("");
  const [sqft, setSqft] = useState(""); const [stories, setStories] = useState("");
  ...
  const toggleFeature = (f: string) => setFeatures((arr) => arr.includes(f) ? arr.filter((x) => x !== f) : [...arr, f]);
  const [garage, setGarage] = useState(""); const [garageSize, setGarageSize] = useState("");
  const [foundation, setFoundation] = useState(""); const [basementType, setBasementType] = useState("");
  const [roofType, setRoofType] = useState(""); const [roofAge, setRoofAge] = useState("");
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await sendQuoteEmail({
        formKind: "Flood Insurance Quote",
        source: "/flood-insurance-quote",
        primaryName: fullName, customerName: fullName, customerEmail: email, customerPhone: phone,
        sections: [
          { title: "Applicant Information", rows: [["Full Name", fullName], ["Address", `${address}, ${city}, ${state} ${zip}`], ["Email", email], ["Phone", phone], ["Date of Birth", dob], ["SSN (Last 4)", ssn4], ["Currently has flood insurance", hasFlood], ["Expiration Date", expiration]] },
          { title: "Coverage Information", rows: [["Coverage requested for", coverageFor], ["Years Lived at Address", yearsLived], ["Dwelling Value", dwellingValue ? `$${dwellingValue}` : ""], ["Dwelling Occupied By", occupiedBy]] },
          { title: "Dwelling Information", rows: [["Building Structure", structure], ["Year Built", yearBuilt], ["Square Feet", sqft], ["Stories", stories], ["Fireplace", fireplace], ["Full Baths", fullBaths], ["Half Baths", halfBaths], ["Bedrooms", bedrooms], ["Additional Features", features.join(", ")], ["Garage", garage], ["Garage Size", garageSize], ["Foundation", foundation], ["Basement Type", basementType], ["Roof Type", roofType], ["Roof Age (years)", roofAge]] },
  ...
  if (status === "done") {
    return (
      <main className="min-h-screen bg-white font-['Inter',sans-serif]">
        <SEO title="Quote Submitted | Custom Insurance Agency" description="Your flood insurance quote has been submitted." />
        <Navbar />
  ...
  return (
    <main className="min-h-screen bg-white font-['Inter',sans-serif]">
      <SEO title="Flood Insurance Quote | Custom Insurance Agency" description="Request a free flood insurance quote online. We'll get back to you within 1 business day." />
      <Navbar />
      <section className="relative flex items-center justify-center pt-28 pb-10 md:pt-32 md:pb-14" style={{ background: "linear-gradient(to bottom, #1f4d7a 0%, #173b5d 60%, #0f2a42 100%)" }}>
        <div className="px-6 text-center text-white">
          <h1 style={{ fontFamily: "'Barlow', sans-serif", fontWeight: 700, fontSize: "clamp(32px, 5vw, 46px)", lineHeight: 1.1 }}>Flood Insurance Quote</h1>
      ...
      <section className="px-4 py-10 md:px-12">
        <form onSubmit={onSubmit} className="mx-auto max-w-[900px] rounded-2xl bg-white p-6" style={{ border: "1px solid #e5e7eb", borderTop: `3px solid ${TEAL}`, boxShadow: "0 8px 30px rgba(0,0,0,0.04)" }}>
          <Section title="1. Applicant Information">
            ...
            <div><Label>Do you currently have flood insurance?</Label><div className="flex items-center gap-5 pt-1">
              <Radio name="hasFlood" value="Yes" current={hasFlood} onChange={setHasFlood} label="Yes" />
              <Radio name="hasFlood" value="No" current={hasFlood} onChange={setHasFlood} label="No" />
            </div></div>
            {hasFlood === "Yes" && (
          ...
          <Section title="2. Coverage Information">
            ...
            <div><Label>Dwelling Occupied By</Label><div className="flex items-center gap-5 pt-1">
              <Radio name="occupiedBy" value="Owner" current={occupiedBy} onChange={setOccupiedBy} label="Owner" />
              <Radio name="occupiedBy" value="Tenant" current={occupiedBy} onChange={setOccupiedBy} label="Tenant" />
            </div></div>
          </Section>
          ...
          <Section title="3. Dwelling Information">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div><Label>Building Structure</Label><div className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-1">
                {["Frame","Masonry","Masonry Veneer"].map((o) => <Radio key={o} name="structure" value={o} current={structure} onChange={setStructure} label={o} />)}
              </div></div>
              ...
              <div><Label>Number of Stories</Label><input type="number" {...fieldProps} value={stories} onChange={(e) => setStories(e.target.value)} /></div>
              <div><Label>Fireplace</Label><div className="flex items-center gap-5 pt-1">
                <Radio name="fireplace" value="Yes" current={fireplace} onChange={setFireplace} label="Yes" />
                <Radio name="fireplace" value="No" current={fireplace} onChange={setFireplace} label="No" />
              </div></div>
              ...
              <div><Label>Additional Features</Label><div className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-1">
                {["Deck","Patio","Porch"].map((f) => (
                  <label key={f} className="inline-flex cursor-pointer items-center gap-2" style={{ fontSize: 13, color: NAVY }}>
                    <input type="checkbox" checked={features.includes(f)} onChange={() => toggleFeature(f)} style={{ accentColor: TEAL }} />{f}
                  </label>
                ))}
              </div></div>
              <div className="md:col-span-2"><Label>Garage</Label><div className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-1">
                {["Attached","Unattached","Built In","Carport","None"].map((o) => <Radio key={o} name="garage" value={o} current={garage} onChange={setGarage} label={o} />)}
              </div></div>
              {garage && garage !== "None" && (
                <div className="md:col-span-2"><Label>Garage Size</Label><input {...fieldProps} value={garageSize} onChange={(e) => setGarageSize(e.target.value)} /></div>
              )}
              <div className="md:col-span-2"><Label>Type of Foundation</Label><div className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-1">
                {["Crawl Space","Slab on Grade","Piers/Pilings/Stilts","Basement"].map((o) => <Radio key={o} name="foundation" value={o} current={foundation} onChange={setFoundation} label={o} />)}
              </div></div>
              {foundation === "Basement" && (
                <div className="md:col-span-2"><Label>Basement</Label><div className="flex items-center gap-5 pt-1">
                  <Radio name="basementType" value="Finished" current={basementType} onChange={setBasementType} label="Finished" />
                  <Radio name="basementType" value="Unfinished" current={basementType} onChange={setBasementType} label="Unfinished" />
                </div></div>
              )}
              <div><Label>Roof Type</Label><div className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-1">
                {["Asphalt Shingles","Wood","Metal"].map((o) => <Radio key={o} name="roofType" value={o} current={roofType} onChange={setRoofType} label={o} />)}
              </div></div>
              <div><Label>Roof Age (years)</Label><input type="number" {...fieldProps} value={roofAge} onChange={(e) => setRoofAge(e.target.value)} /></div>
            </div>
          </Section>
