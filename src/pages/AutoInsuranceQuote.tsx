...
type Vehicle = { make: string; model: string; year: string; vin: string };
const blankVehicle = (): Vehicle => ({ make: "", model: "", year: "", vin: "" });
...
export default function AutoInsuranceQuote() {
  ...
  const updateDriver = (i: number, k: keyof Driver, v: string) =>
    setDrivers((arr) => arr.map((d, idx) => (idx === i ? { ...d, [k]: v } : d)));
  ...
  const updateVehicle = (i: number, k: keyof Vehicle, v: string) =>
    setVehicles((arr) => arr.map((d, idx) => (idx === i ? { ...d, [k]: v } : d)));
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    const validVehicles = vehicles.filter((v) => v.make.trim() && v.model.trim());
    if (!validVehicles.length) return toast.error("Please add at least one vehicle with make and model.");
    ...
    try {
      ...
      const driverRows = validDrivers.map(
        (d, i) =>
          [
            `Driver ${i + 1}`,
            `Name: ${d.name} | License: ${d.license || "—"} | DOB: ${d.dob || "—"} | SSN (last 4): ${d.ssnLast4 || "—"} | Accidents: ${d.accidents || "0"} | Violations: ${d.violations || "0"} | Suspension: ${d.suspension || "—"}`,
          ] as [string, string],
      );

      const vehicleRows = validVehicles.map(
        (v, i) =>
          [
            `Vehicle ${i + 1}`,
      ...
      await sendQuoteEmail({
        ...
        sections: [
          ...
          { title: "Vehicle Information", rows: vehicleRows },
          {
  ...
  if (status === "done") {
    return (
      <main className="min-h-screen bg-white font-['Inter',sans-serif]">
        <SEO title="Quote Submitted | Custom Insurance Agency" description="Your auto insurance quote request has been submitted." />
        <Navbar />
  ...
  return (
    <main className="min-h-screen bg-white font-['Inter',sans-serif]">
      <SEO
        title="Auto Insurance Quote | Custom Insurance Agency"
        description="Request a free auto insurance quote online. We'll get back to you within 1 business day."
      />
      <Navbar />
        ...
        >
          ...
          <Section title="2. Driver Information">
            <p style={{ fontSize: 12, color: "#4a5568", marginTop: -4 }}>
              List all drivers. Include accidents, moving violations, or suspensions in the last 3 years.
            </p>
            {drivers.map((d, i) => (
              <div
                key={i}
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    ...
                    <input {...fieldProps} value={d.name} onChange={(e) => updateDriver(i, "name", e.target.value)} />
                  </div>
                  <div>
                    <Label>Driver's License #</Label>
                    <input {...fieldProps} value={d.license} onChange={(e) => updateDriver(i, "license", e.target.value)} />
                  </div>
                  <div>
                    <Label>Date of Birth</Label>
                    <input type="date" {...fieldProps} value={d.dob} onChange={(e) => updateDriver(i, "dob", e.target.value)} />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      maxLength={4}
                      value={d.ssnLast4}
                      onChange={(e) => updateDriver(i, "ssnLast4", e.target.value.replace(/\D/g, ""))}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      {...fieldProps}
                      value={d.accidents}
                      onChange={(e) => updateDriver(i, "accidents", e.target.value)}
                    />
                  </div>
                  <div>
                    <Label>MV Violations</Label>
                    <input
                      type="number"
                      min={0}
                      {...fieldProps}
                      value={d.violations}
                      onChange={(e) => updateDriver(i, "violations", e.target.value)}
                    />
                  </div>
                  <div className="sm:col-span-2">
                    ...
                    <div className="flex items-center gap-5 pt-1">
                      <Radio
                        name={`susp-${i}`}
                        value="Yes"
                        current={d.suspension}
                        onChange={(v) => updateDriver(i, "suspension", v)}
                        label="Yes"
                      />
                      <Radio
                        name={`susp-${i}`}
            ...
            <button
              type="button"
              onClick={() => setDrivers((arr) => [...arr, blankDriver()])}
          ...
          {/* Section 3 */}
          <Section title="3. Vehicle Information">
            {vehicles.map((v, i) => (
              <div
                key={i}
                className="relative rounded-lg p-4"
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <Label>Make</Label>
                    <input {...fieldProps} value={v.make} onChange={(e) => updateVehicle(i, "make", e.target.value)} />
                  </div>
                  <div>
                    <Label>Model</Label>
                    <input {...fieldProps} value={v.model} onChange={(e) => updateVehicle(i, "model", e.target.value)} />
                  </div>
                  <div>
                    <Label>Year</Label>
                    <input
                      ...
                      {...fieldProps}
                      value={v.year}
                      onChange={(e) => updateVehicle(i, "year", e.target.value.slice(0, 4))}
                    />
                  </div>
                  <div>
                    <Label>VIN</Label>
                    <input {...fieldProps} value={v.vin} onChange={(e) => updateVehicle(i, "vin", e.target.value)} />
                  </div>
                </div>
              </div>
            ))}
            <button
              type="button"
              onClick={() => setVehicles((arr) => [...arr, blankVehicle()])}
              className="inline-flex items-center gap-2 self-start rounded-md px-4 py-2"
              style={{ border: `1px solid ${TEAL}`, color: TEAL, fontSize: 13, fontWeight: 600 }}
            >
