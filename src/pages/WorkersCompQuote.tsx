...
type Employee = { name: string; code: string; payroll: string };
const blankEmp = (): Employee => ({ name: "", code: "", payroll: "" });
...
export default function WorkersCompQuote() {
  ...
  // Employees
  const [employees, setEmployees] = useState<Employee[]>([blankEmp()]);
  const updateEmp = (i: number, k: keyof Employee, v: string) =>
    setEmployees((arr) => arr.map((e, idx) => (idx === i ? { ...e, [k]: v } : e)));
  const addEmp = () => setEmployees((arr) => [...arr, blankEmp()]);
  const removeEmp = (i: number) => setEmployees((arr) => (arr.length > 1 ? arr.filter((_, idx) => idx !== i) : arr));
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    const validEmps = employees.filter((emp) => emp.name.trim() || emp.code.trim() || emp.payroll.trim());
    if (!validEmps.length) return toast.error("Add at least one employee.");
    ...
    try {
      ...
      await sendQuoteEmail({
        formKind: "Workers Compensation Quote",
        source: "/workers-comp-quote",
        primaryName: bizName,
        ...
        sections: [
          ...
          },
          {
            ...
            rows: validEmps.map((emp, i) => [
              `Employee ${i + 1}`,
              `Name: ${emp.name || "—"} | Class Code: ${emp.code || "—"} | Yearly Payroll: ${emp.payroll ? `$${emp.payroll}` : "—"}`,
            ]),
          },
          {
  ...
  if (status === "done") {
    return (
      <main className="min-h-screen bg-white font-['Inter',sans-serif]">
        <SEO title="Workers Compensation Quote Submitted | Custom Insurance Agency" description="Your workers compensation quote request has been submitted." />
        <Navbar />
  ...
  return (
    <main className="min-h-screen bg-white font-['Inter',sans-serif]">
      <SEO
        title="Workers Compensation Quote | Custom Insurance Agency"
        description="Request a free workers compensation insurance quote from Custom Insurance Agency."
      />
      <Navbar />
        ...
        >
          <Section title="1. Business Information">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                <Label>Federal Employer's ID #</Label>
                <input value={fein} onChange={(e) => setFein(e.target.value)} {...fieldProps} />
              </div>
              <div>
          ...
          <Section title="2. Employee Information">
            <p style={{ color: NAVY, fontSize: 13, fontFamily: "Inter, sans-serif" }}>
              List employees with their classification codes and estimated yearly payroll.
            </p>
            {employees.map((emp, i) => (
              <div key={i} className="flex items-end gap-2">
                <div className="grid flex-1 grid-cols-1 gap-2 md:grid-cols-3">
                  <div>
                    ...
                    <input value={emp.name} onChange={(ev) => updateEmp(i, "name", ev.target.value)} {...fieldProps} />
                  </div>
                  <div>
                    <Label>Classification Code</Label>
                    <input value={emp.code} onChange={(ev) => updateEmp(i, "code", ev.target.value)} {...fieldProps} />
                  </div>
                  <div>
                    <Label>Yearly Payroll Estimate</Label>
                    <DollarInput value={emp.payroll} onChange={(v) => updateEmp(i, "payroll", v)} />
                  </div>
                </div>
                <button
                  type="button"
                  ...
                  style={{
                    height: 36, width: 36, borderRadius: 6, border: "1px solid #e5e7eb",
                    background: "#fff", color: "#dc2626", display: "inline-flex",
          ...
          <Section title="3. Signature & Authorization">
