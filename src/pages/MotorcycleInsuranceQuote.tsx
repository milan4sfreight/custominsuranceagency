...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-4">
      <h3 className="mb-3 pb-2 uppercase" style={{ color: NAVY, fontSize: 13, letterSpacing: "0.08em", borderBottom: "1px solid #e5e7eb", fontWeight: 700, fontFamily: "Inter, sans-serif" }}>
        {title}
      </h3>
...
type Moto = { make: string; model: string; year: string; vin: string };
const blankMoto = (): Moto => ({ make: "", model: "", year: "", vin: "" });
...
export default function MotorcycleInsuranceQuote() {
  ...
  const [motos, setMotos] = useState<Moto[]>([blankMoto()]);
  const updateMoto = (i: number, k: keyof Moto, v: string) => setMotos((arr) => arr.map((d, idx) => (idx === i ? { ...d, [k]: v } : d)));
  const removeMoto = (i: number) => setMotos((arr) => arr.filter((_, idx) => idx !== i));
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    const validMotos = motos.filter((v) => v.make.trim() && v.model.trim());
    if (!validMotos.length) return toast.error("Please add at least one motorcycle with make and model.");
    ...
    try {
      ...
      const motoRows = validMotos.map((v, i) => [`Motorcycle ${i + 1}`, `${v.year || "—"} ${v.make} ${v.model} | VIN: ${v.vin || "—"}`] as [string, string]);

      await sendQuoteEmail({
        formKind: "Motorcycle Insurance Quote",
        source: "/motorcycle-insurance-quote",
        ...
        sections: [
          ...
          { title: "Motorcycle Information", rows: motoRows },
  ...
  if (status === "done") {
    return (
      <main className="min-h-screen bg-white font-['Inter',sans-serif]">
        <SEO title="Quote Submitted | Custom Insurance Agency" description="Your motorcycle insurance quote request has been submitted." />
        <Navbar />
  ...
  return (
    <main className="min-h-screen bg-white font-['Inter',sans-serif]">
      <SEO title="Motorcycle Insurance Quote | Custom Insurance Agency" description="Request a free motorcycle insurance quote online. We'll get back to you within 1 business day." />
      <Navbar />
      <section className="relative flex items-center justify-center pt-28 pb-10 md:pt-32 md:pb-14" style={{ background: "linear-gradient(to bottom, #1f4d7a 0%, #173b5d 60%, #0f2a42 100%)" }}>
        <div className="px-6 text-center text-white">
          <h1 style={{ fontFamily: "'Barlow', sans-serif", fontWeight: 700, fontSize: "clamp(32px, 5vw, 46px)", lineHeight: 1.1 }}>Motorcycle Insurance Quote</h1>
      ...
      <section className="px-4 py-10 md:px-12">
        <form onSubmit={onSubmit} className="mx-auto max-w-[900px] rounded-2xl bg-white p-6" style={{ border: "1px solid #e5e7eb", borderTop: `3px solid ${TEAL}`, boxShadow: "0 8px 30px rgba(0,0,0,0.04)" }}>
          ...
          <Section title="3. Motorcycle Information">
            <p style={{ fontSize: 12, color: "#4a5568", marginTop: -4 }}>List all motorcycles to be insured.</p>
            {motos.map((v, i) => (
              <div key={i} className="relative rounded-lg p-4" style={{ border: "1px solid #e5e7eb", background: "#fafbfc" }}>
                <div className="mb-2 flex items-center justify-between">
                  <span style={{ fontSize: 12, fontWeight: 700, color: NAVY, letterSpacing: "0.05em" }}>MOTORCYCLE {i + 1}</span>
                  {motos.length > 1 && (
                    <button type="button" onClick={() => removeMoto(i)} className="flex h-6 w-6 items-center justify-center rounded-full" style={{ background: "#fee2e2", color: "#dc2626" }} aria-label="Remove motorcycle"><X size={14} /></button>
                  )}
                </div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div><Label>Make</Label><input {...fieldProps} value={v.make} onChange={(e) => updateMoto(i, "make", e.target.value)} /></div>
                  <div><Label>Model</Label><input {...fieldProps} value={v.model} onChange={(e) => updateMoto(i, "model", e.target.value)} /></div>
                  <div><Label>Year</Label><input type="number" {...fieldProps} value={v.year} onChange={(e) => updateMoto(i, "year", e.target.value.slice(0, 4))} /></div>
                  <div><Label>VIN</Label><input {...fieldProps} value={v.vin} onChange={(e) => updateMoto(i, "vin", e.target.value)} /></div>
                </div>
              </div>
            ))}
            <button type="button" onClick={() => setMotos((arr) => [...arr, blankMoto()])} className="inline-flex items-center gap-2 self-start rounded-md px-4 py-2" style={{ border: `1px solid ${TEAL}`, color: TEAL, fontSize: 13, fontWeight: 600 }}>
              <Plus size={14} /> Add Motorcycle
            </button>
          </Section>
