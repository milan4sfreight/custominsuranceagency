...
type Driver = { name: string; license: string; dob: string; ssnLast4: string; accidents: string; violations: string; suspension: "Yes" | "No" | "" };
const blankDriver = (): Driver => ({ name: "", license: "", dob: "", ssnLast4: "", accidents: "0", violations: "0", suspension: "" });
type RV = { make: string; model: string; year: string; modelLength: string; vin: string };
const blankRV = (): RV => ({ make: "", model: "", year: "", modelLength: "", vin: "" });

export default function RVInsuranceQuote() {
  ...
  const [rvs, setRvs] = useState<RV[]>([blankRV()]);
  const updateRV = (i: number, k: keyof RV, v: string) => setRvs((arr) => arr.map((d, idx) => (idx === i ? { ...d, [k]: v } : d)));
  const removeRV = (i: number) => setRvs((arr) => arr.filter((_, idx) => idx !== i));
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    const validRVs = rvs.filter((v) => v.make.trim() && v.model.trim());
    if (!validRVs.length) return toast.error("Please add at least one RV with make and model.");
    ...
    try {
      ...
      const rvRows: Array<[string, string]> = [["Type of RV", rvType || "—"]];
      validRVs.forEach((v, i) => rvRows.push([`Vehicle ${i + 1}`, `${v.year || "—"} ${v.make} ${v.model} | Length: ${v.modelLength || "—"} | VIN: ${v.vin || "—"}`]));

      await sendQuoteEmail({
        formKind: "RV Insurance Quote",
        source: "/rv-insurance-quote",
        ...
        sections: [
          ...
          { title: "RV Information", rows: rvRows },
  ...
  if (status === "done") {
    return (
      <main className="min-h-screen bg-white font-['Inter',sans-serif]">
        <SEO title="Quote Submitted | Custom Insurance Agency" description="Your RV insurance quote request has been submitted." />
        <Navbar />
  ...
  return (
    <main className="min-h-screen bg-white font-['Inter',sans-serif]">
      <SEO title="RV Insurance Quote | Custom Insurance Agency" description="Request a free RV insurance quote online. We'll get back to you within 1 business day." />
      <Navbar />
      <section className="relative flex items-center justify-center pt-28 pb-10 md:pt-32 md:pb-14" style={{ background: "linear-gradient(to bottom, #1f4d7a 0%, #173b5d 60%, #0f2a42 100%)" }}>
        <div className="px-6 text-center text-white">
          <h1 style={{ fontFamily: "'Barlow', sans-serif", fontWeight: 700, fontSize: "clamp(32px, 5vw, 46px)", lineHeight: 1.1 }}>RV Insurance Quote</h1>
      ...
      <section className="px-4 py-10 md:px-12">
        <form onSubmit={onSubmit} className="mx-auto max-w-[900px] rounded-2xl bg-white p-6" style={{ border: "1px solid #e5e7eb", borderTop: `3px solid ${TEAL}`, boxShadow: "0 8px 30px rgba(0,0,0,0.04)" }}>
          ...
          <Section title="3. RV Information">
            <p style={{ fontSize: 12, color: "#4a5568", marginTop: -4 }}>List all RVs to be insured.</p>
            <div>
              <Label>Type of RV</Label>
              <div className="flex items-center gap-5 pt-1">
                <Radio name="rvType" value="Motor Home" current={rvType} onChange={setRvType} label="Motor Home" />
                <Radio name="rvType" value="Travel Trailer" current={rvType} onChange={setRvType} label="Travel Trailer" />
              </div>
            </div>
            {rvs.map((v, i) => (
              <div key={i} className="relative rounded-lg p-4" style={{ border: "1px solid #e5e7eb", background: "#fafbfc" }}>
                <div className="mb-2 flex items-center justify-between">
                  <span style={{ fontSize: 12, fontWeight: 700, color: NAVY, letterSpacing: "0.05em" }}>VEHICLE {i + 1}</span>
                  {rvs.length > 1 && (
                    <button type="button" onClick={() => removeRV(i)} className="flex h-6 w-6 items-center justify-center rounded-full" style={{ background: "#fee2e2", color: "#dc2626" }} aria-label="Remove RV"><X size={14} /></button>
                  )}
                </div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div><Label>Make</Label><input {...fieldProps} value={v.make} onChange={(e) => updateRV(i, "make", e.target.value)} /></div>
                  <div><Label>Model</Label><input {...fieldProps} value={v.model} onChange={(e) => updateRV(i, "model", e.target.value)} /></div>
                  <div><Label>Year</Label><input type="number" {...fieldProps} value={v.year} onChange={(e) => updateRV(i, "year", e.target.value.slice(0, 4))} /></div>
                  <div><Label>Model Length</Label><input {...fieldProps} placeholder="32 ft" value={v.modelLength} onChange={(e) => updateRV(i, "modelLength", e.target.value)} /></div>
                  <div className="sm:col-span-2"><Label>VIN</Label><input {...fieldProps} value={v.vin} onChange={(e) => updateRV(i, "vin", e.target.value)} /></div>
                </div>
              </div>
            ))}
            <button type="button" onClick={() => setRvs((arr) => [...arr, blankRV()])} className="inline-flex items-center gap-2 self-start rounded-md px-4 py-2" style={{ border: `1px solid ${TEAL}`, color: TEAL, fontSize: 13, fontWeight: 600 }}>
              <Plus size={14} /> Add RV
            </button>
          </Section>
